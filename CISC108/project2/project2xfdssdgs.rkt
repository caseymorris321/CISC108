...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname project2xfdssdgs) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define-struct snake (segs dir alive?))

#;
(define (snake-template snake)
  ... (snake-segs snake) ... (snake-dir snake) ...)

; a Dir is one of: 'up, 'down, 'left, 'righ
...
(define snake1 (make-snake segs1 "up" #true))
(define snake2 (make-snake segs2 "up" #true))
...
(define (new-seg seg dir)
   (cond [(string=? dir "up") (make-posn (posn-x seg) (+ 1 (posn-y seg)))]
         [(string=? dir "down") (make-posn (posn-x seg) (- (posn-y seg) 1))]
         [(string=? dir "left") (make-posn (- (posn-x seg) 1) (posn-y seg))]
...
(check-expect (snake-slither snake2)
              (make-snake (list (make-posn 2 6) (make-posn 2 5)) "up" #true))
...
(check-expect (self-collision?
               (make-world
                (make-snake (list (make-posn 1 2) (make-posn 1 2)) "up" #true) food1))
              true)
...
(check-expect (world-collision?
               (make-world (make-snake (list (make-posn -1 2)) "up" #true) food1))
              true)
